...
/// Compact-width (iPhone) experience: standard push navigation.
struct BoardListCompactView: View {
  @ObservedObject var model: BoardViewModel
  @StateObject private var tracker = TrackedServiceManager.shared
          ...
          if let uid = item.serviceUid, let date = item.runDate, let ld = item.locationDetail {
            ...
            .swipeActions(edge: .trailing, allowsFullSwipe: false) {
              if !model.showingArrivals {
                Button {
                  Task {
                    if tracker.isTracking(serviceUid: uid, runDate: date) {
                      await tracker.stopTracking()
                    } else {
                      let dest = ld.destination?.first?.description ?? L10n.emDash
                      let booked = ld.gbttBookedDeparture ?? ""
                      await tracker.startTracking(
                        serviceUid: uid,
                        runDate: date,
                        stationName: model.stationDesc,
                        stationCRS: model.stationCRS,
                  ...
                  Label(
                    tracker.isTracking(serviceUid: uid, runDate: date) ? L10n.unpin : L10n.pin,
                    systemImage: "pin"
                  )
                }
                .tint(.blue)
